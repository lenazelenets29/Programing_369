...
int main(int argc, char const *argv[]) {
    FILE *in_t;
    FILE *out_t; // создаем входящий и выходящий файл
    ...
    int N = atoi(&c_N[0]); // преобразовуем в int
    char mat[2*N*N + 1]; // обьявляем матрицу
    for(int i = 0; i < 2*N*N; i++){
        mat[i] = getc(in_t);
    } // функция принимает матрицу с входного файла
    mat[2*N*N-1] = '\n'; // добавляем перенос строки в конце матрицы
    mat[2*N*N] = 0; // указываем окончание строки
    if((out_t = fopen("out.txt", "w"))==NULL) {
        printf("Cannot open file.\n");
        exit(1);
    } // открываем выходящий файл

    for(int i = 0; i < 2*N*N; i++){
        putc(mat[i], out_t);
    } // выводим в выходной файл матрицу
    printf(mat); // выводим ее на экран
    int arr[N][N];
    for (int i = 0; i < N; i++){
        int k = 2 * i * N;
    ...
    } // сортируем диагональ матрицы
    char a_t[2*N]; // создаем строку для отсортированной диагонали
    for(int i = 0; i < 2*N; i = i +2){
        itoa(a[i/2], &a_t[i], 10);
        a_t[i + 1] = *" "; 
    } // преобразовуем в char и записываем с учетом пробелов
    a_t[2*N-1] = 0; // указываем на конец строки
    putc('\n', out_t); // выводим перенос строки в выходной файл
    for(int i = 0; i < 2*N - 1; i++){
        putc(a_t[i], out_t);
    } // выходной файл принимает массив диагонали и записывает ее
    fclose(in_t);
    fclose(out_t); // закрываем оба файла
    return 0;
}